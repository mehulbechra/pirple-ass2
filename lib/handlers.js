...
handlers.sessionCreate = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    const templateData = {
      'head.title': 'Log in',
      'head.description': 'Sign in to order a pizza',
...
handlers.tokens = (data, callback) => {
  const acceptableMethods = ['post', 'delete', 'get', 'put'];
    ...
    _data.read('tokens', id, (readErr, tokenData) => {
      if (!readErr && tokenData) {
        ...
        if (tokenData.expires > Date.now()) {
          ...
          _data.update('tokens', id, tokenData, (updateErr) => {
            if (!updateErr) {
              callback(200);
            } else {
              ...
              if (tokenIsValid) {
                ...
                if (userData.cart.length > 0) {
                  ...
                  userData.cart.forEach((orderItem) => {
                    ...
                    orderObject.order[orderItem.itemId] = orderItem.quantity;
                  });
                  orderObject.orderTotal = amountInCents / 100;

                  // Creating payment
                  helpers.createPayment(amountInCents, description, stripeToken, (paymentErr) => {
                    if (!paymentErr) {
                      _data.create('orders', orderId, orderObject, (createErr) => {
                        if (!createErr) {
                          // Empty the cart
                          userData.cart = [];
                          _data.update('users', userEmail, userData, (updateErr) => {

                            ...
                            if (tokenIsValid) {
                                ...
                                _data.read('menu', 'menu', (err, menuData) => {
                                    if (!err && menuData) {
                                        ...
                                        const itemObj = menuData.find(item => {
                                            return item.itemId == itemId;
                                        });
                                        if(itemObj){
                                            // Check if itemObj exists in cart
                                            const userItemObj = cart.find(item => { return item.itemId == itemId; });
                        ...
                        handlers.tokens.verifyToken(token, userEmail, (tokenIsValid) => {
                            if (tokenIsValid) {
                                ...
                                if(userData.cart.length > 0){
                                    ...
                                    // Creating payment
                                    helpers.createPayment(amountInCents, description, stripeToken, (err) => {
                                        if(!err) {
                                            // Empty the cart
                                            userData.cart = [];
                                            _data.update('users',userEmail,userData,(err)=>{
